...
class AlipayNotify{
	...
	function verifyReturn(){
		if(empty(Input::get())) {//判断POST来的数组是否为空
			return false;
		} else {
			//生成签名结果
			$isSign = $this->getSignVeryfy(Input::get(), Input::get('sign'));
			//获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
			$responseTxt = 'false';
			if (!empty(Input::get('notify_id')))
			{
				$responseTxt = $this->getResponse(Input::get('notify_id'));
			}
